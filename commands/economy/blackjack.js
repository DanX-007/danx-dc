...
module.exports = {
    ...
    run: async ({ interaction, client }) => {
        ...
        try {
            ...
            // Simpan collector di interaction untuk diakses di handler
            ...
            collector.on('collect', async i => {
                try {
                    ...
                    // Jika collector seharusnya sudah berhenti, abaikan interaksi
                    if (!interaction.collector) return;
                    ...
                    switch (i.customId) {
                        case 'hit':
                            ...
                            break;
                        case 'stand':
                            await handleStand(i, gameState, userEconomy, balance, type);
                            interaction.collector = null;
                            return;
                        case 'double':
                    ...
                    if (calculateHandValue(gameState.playerHand) > 21) {
                        await handleBust(i, gameState, userEconomy, balance, type);
                        interaction.collector = null;
                        return;
                    }
...
async function handleStand(interaction, gameState, userEconomy, balance, type) {
    try {
        ...
        // Handle insurance
        if (gameState.insurance > 0) {
            if (dealerValue === 21 && gameState.dealerHand[0].card === 'A') {
                // Player menang insurance
                const insuranceWin = gameState.insurance * 2;
                balance += insuranceWin;
                embed.addFields({ name: 'Insurance', value: `+${insuranceWin} coins (Dealer has Blackjack)` });
            } else {
                // Player kalah insurance
                embed.addFields({ name: 'Insurance', value: `-${gameState.insurance} coins (Dealer does not have Blackjack)` });
            }
        }
        ...
        // Hentikan collector sebelum mengedit reply
        if (interaction.collector) {
            interaction.collector.stop();
        }
